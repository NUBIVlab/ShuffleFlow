"""
    ...
    Adpated from https://github.com/devzhk/InverseBench/blob/main/utils/diffusion.py.
"""
...
class DiffusionSampler(BaseSampler):
    def __init__(self, img_shape, net, forward_op, diffusion_scheduler_config, sde=False, **kwargs):
        super(DiffusionSampler, self).__init__(img_shape, net, forward_op, **kwargs)
    ...
    def __call__(self, observation, num_samples=1, verbose=True):
        ...
        xt = torch.randn(num_samples, *self.img_shape, device=self.device) * diffusion_scheduler.sigma_max
        sampler = ReverseSampler(diffusion_scheduler)
        xt = sampler.sample(self.net, xt, SDE=self.sde, verbose=verbose)
        return xt