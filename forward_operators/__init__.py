...
class BaseOperator(ABC):
    def __init__(self, sigma_noise=0.0, unnorm_shift=0.0, unnorm_scale=1.0, device='cuda', **kwargs):
    ...
    def __call__(self, x, **kwargs):
        measurement = self.forward(x, **kwargs)
        return measurement + self.sigma_noise * torch.randn_like(measurement)
    ...
    def gradient(self, pred, observation, return_loss=False):
        ...
        pred_tmp = self.unnormalize(pred_tmp)
        loss = self.loss(pred_tmp, observation)
        pred_grad = grad(loss.sum(), pred_tmp)[0]
        if return_loss:
    ...
    def loss(self, pred, observation, **kwargs):
        ...
        """
        # print(pred.flatten(start_dim=1).shape, observation.flatten(start_dim=1).shape)
...
from .single_coil_mri import SingleCoilMRI
