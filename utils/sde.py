  ...
  def reverse(self, score_fn, probability_flow=False):
    ...
    class RSDE(self.__class__):
      ...
      def sde(self, x, t):
        ...
        score =  score_fn(x, t)
        drift = drift - diffusion.view(-1,1,1,1) ** 2 * score * (0.5 if self.probability_flow else 1.)
      ...
      def discretize(self, x, t):
        ...
        rev_f = f - G.view(-1,1,1,1) ** 2 * score_fn(x, t) * (0.5 if self.probability_flow else 1.)
...
class VPSDE(SDE):
  def __init__(self, beta_min=0.1, beta_max=20, N=1000):
    ...
    self.N = N
    self.timesteps = torch.linspace(1/self.N, 1.0, self.N)
  ...
  def prior_logp(self, z):
    N = np.prod(z.shape[-3:])
    logps = - N / 2. * np.log(2 * np.pi) - torch.sum(z ** 2, dim=(-3,-2,-1)) / 2.
    return logps
